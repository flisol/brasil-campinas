...
##########################################
# Autoloading (almost) everything
##########################################
define('NS', '\\');
define('FS', '_');
define('PS', PATH_SEPARATOR);

spl_autoload_extensions(SYS_MODEL_EXT . ',' . SYS_VIEW_EXT . ',' . SYS_CONTROLLER_EXT. ',' . SYS_SPICES_EXT);
spl_autoload_register(
    function($className) {
  
        $path = str_replace(NS, DS, $className);
        $file = sprintf("%s", $path);
        
        foreach (explode(PS, get_include_path()) as $path) {
            $curPath = trim($path) . DS . $file;
                                    
            foreach(explode(',',spl_autoload_extensions()) as $ext) {
              trim($ext);
              if (file_exists($curPath . $ext)) {
                  // Load the file
                  require_once( $curPath . $ext );
              }
            }
        }
    }
